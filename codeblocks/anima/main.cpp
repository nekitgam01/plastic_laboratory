	...
	public:
		...
		void Draw(){
			...
			if (Room == 0) {
				...
				if (nScreenSize==0){ //800x600
					Panel1.Draw(195,155);
					Button0.Draw(203,165);
					DrawWord(screen,1,"Hfphtitybt^ 800[600",212,175,cWHITE);
					DrawWord(screen,1,"Dckexft ghj,ktv c yfcnhjqrfvb? lkz c,hjcf dctulf hf,jnftn ryjgrf",10,585,cRED);
					DrawWord(screen,0,"[F11]",464,585,cRED);
				}
				...
				else if (nScreenSize==4){ //1366x768
					Panel1.Draw(506,210);
		...
		void MouseDown(){
			//If mouse click in ButtonStart - start game 
			if (nSelected == 0){
				Room = 1;
			//Else if click in button Settings - open settings room
			} else if (nSelected == 1){
				Room = 99;
			}
		};
		void KeyDown(int nKey){
			//If Key = "m" then open map editor
			Room = nKey;
		};
		void Destroy(){
			delete this;
